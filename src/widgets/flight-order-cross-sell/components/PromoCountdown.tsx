    ...
    >
      {values.map((value, index) => (
        <div className="flex items-center gap-1" key={countdownLabels[index]}>
          <span className="flex h-8 min-w-9 items-center justify-center bg-(--csc-countdown-bg) px-2 text-center text-base leading-8 font-bold text-foreground">
            {formatCountdownUnit(value)}
          </span>
          <span className="text-xs leading-5 text-(--csc-text-subtle)">
