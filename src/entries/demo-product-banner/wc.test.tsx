...
vi.mock('../../widgets/demo-product-banner', async (importOriginal) => {
  const actual = await importOriginal<object>()

  return {
    ...actual,
    DemoProductBanner: (props: unknown) => {
...
type DemoProductBannerElement = HTMLElement & {
  version?: string
...
async function renderElement(element: HTMLElement) {
  ...
  return widgetMockState.props as DemoProductBannerProps
}

describe('demo-product-banner Web Component 契約', () => {
  afterEach(async () => {
    await act(async () => {
  ...
  it('會維持公開 observed attributes 不變', () => {
    const elementConstructor = customElements.get('demo-product-banner') as
  ...
  it('會從 attributes 解析公開 widget props', async () => {
    ...
    element.setAttribute('title', '客製推薦')
